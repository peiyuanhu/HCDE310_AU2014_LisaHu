...
from twisted.protocols.amp import ERROR_CODE
...
reason = airport_data['status']['reason']
print "Airport: " + name + " ("+ city + ", " + state + ")"
print "Reason: " +reason
...
def getAirport(airport_name):
    airport_request = baseurl+airport_name+"?"+param_str
    airport_json_str = urllib2.urlopen(airport_request).read()
    return json.loads(airport_json_str)
